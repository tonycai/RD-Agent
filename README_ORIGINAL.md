...
### 🚀 Deployment
- You can run the following command to deploy the OpenAI-compatible RESTful API.
  ```sh
  uvicorn rdagent.app.gateway.main:app --host 0.0.0.0 --port 8000
  ```
- To start a data science job, you can send a POST request to the `/v1/chat/completions` endpoint with the competition name in the model field.
    ```sh
    curl -X POST "http://localhost:8000/v1/chat/completions" -H "Content-Type: application/json" -d '{
        "model": "rd-agent-sf-crime",
        "messages": [{"role": "user", "content": "Start the agent."}],
        "stream": true
    }'
    ```
